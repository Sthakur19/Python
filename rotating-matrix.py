...
# Rotating a Matrix
# Write a Python function that takes a 2D matrix as input and returns a
# new matrix that is obtained by rotating the original matrix 180 degrees
# clockwise. For example:

# Input matrix
# matrix = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
# Rotated matrix (180 degrees clockwise)
# rotated_matrix = [[9, 8, 7], [6, 5, 4], [3, 2, 1]]
...
def rotated_matrix(matrix):
    ...
    for i in range(matrix_row):
        for j in range(matrix_col):
            rotated_matrix_full[i][j] = matrix[matrix_row - i -1 ][matrix_row - j - 1]
